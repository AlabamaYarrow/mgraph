...
nodes_file = os.path.join(DUMPS_DIR, 'reldoc', 'nodes.csv')
edgenodes_file = os.path.join(DUMPS_DIR, 'reldoc', 'edgenodes.csv')
meta_file = os.path.join(DUMPS_DIR, 'reldoc', 'meta_relations.csv')
...
def write_metas(start_metanid=1, total_metanodes=TOTAL_METANODES, meta_width=META_WIDTH, meta_depth=META_DEPTH):
    ...
    with open(nodes_file, 'a') as f, open(meta_file, 'a+') as gf:
